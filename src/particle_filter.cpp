...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
  ...
  // Declaring initial variables of x, y and theta
  double x_i, y_i, theta_i; 
  
  // Declaring final values which will be evaluated using formula. (These will be ideal case values ignoring uncertainity)
  ...
  for (unsigned int i =0; i<particles.size(); i++){
    ...
    //Extracting the initial values of x, y and theta: 
    x_i = particles[i].x;
    y_i = particles[i].y;  
    ...
    x_f = dist_x(gen);   // Introducing uncertainity in theta
    y_f = dist_y(gen);   // Introducing uncertainity in theta
      
    //Overwriting new values of x, y, theta for our particles
    particles[i].x = x_f;
    particles[i].y = y_f; 
