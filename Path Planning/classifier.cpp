...
void GNB::train(const vector<vector<double>> &data_raw, 
   ...
   */
  vector<vector<double>> data = data_raw;
  
  for (unsigned int i =0; i< data.size(); i++){
      //cout<<"Old d: "<<data[i][1]<<endl;
      data[i][1] = fmod(data[i][1] , 4);
      if (data[i][1]<0) {
          data[i][1]+=4;
      }
      //cout<<"New d: "<<data[i][1]<<endl<<endl;
  }
  ...
  cout<<"sigma_left: "<<left_sds<<endl<<"mu_left: "<<left_means<<endl;    
...
string GNB::predict(const vector<double> &sample_raw) {
  /**
  ...
  vector<double> sample = sample_raw;
